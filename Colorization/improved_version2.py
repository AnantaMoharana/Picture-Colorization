...
def improved_agent(leftHalfColor, leftHalfGrey, rightHalfGrey):
    ...
    for _ in range(1):
        ...
        for training in training_set:
            ...
            training_input=training_input/np.amax(training_input, axis=0)
        ...
        print(np.sum((goal-training_outputs)**2))
        #print((np.sum(goal)-np.sum(training_outputs))**2)
...
def backward_propragation(output, actual,weights1,weights2,training_input,first_layer):
    ...
    weights2+=change_weight2

    weights1+=change_weight1
...
def get_model_training_set(color,leftHalfGrey):
    ...
    for x in range(1, color.shape[0]-1):
        for y in range(1, color.shape[1]-1):
            ...
            actual=color[x][y]
            patches=[
                [upperLeft[0] , upperMid[0] , upperRight[0] , midLeft[0] , midRight[0] , lowerRight[0] , lowerMid[0] , lowerLeft[0] , mid[0] ],
                 [upperLeft[1] , upperMid[1] , upperRight[1] , midLeft[1] , midRight[1] , lowerRight[1] , lowerMid[1] , lowerLeft[1] , mid[1] ],
                 [upperLeft[2] , upperMid[2] , upperRight[2] , midLeft[2] , midRight[2] , lowerRight[2] , lowerMid[2] , lowerLeft[2] , mid[2] ]
            ]
